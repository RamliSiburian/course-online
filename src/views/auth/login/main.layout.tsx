import { Col, Divider, Form, notification, Row, Spin } from 'antd'
...
export default function LoginPage(): React.JSX.Element {
    ...
    const loadingGoogle = isLoading('loginGoogle') || false
    ...
    useEffect(() => {
        if (session) {
            useActions<'loginGoogle'>('loginGoogle', [session?.id, (status: number, user: string) => {
                if (status == 200) {
                    notification.success({
                        description: `Wellcome ${user}`,
                        message: 'Login berhasil',
